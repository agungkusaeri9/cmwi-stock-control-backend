...
export class JsService {
  static async create(filePath: string) {
    ...
    const importantHeaders: string[] = ["Code", "Ending"];
    ...
    try {
      ...
      const [validRequest, missingCodes] = createRequest.reduce<
        [typeof createRequest, string[]]
      >(
        ([valid, missing], js) => {
          existingCodes.has(js.code) ? valid.push(js) : missing.push(js.code);
      ...
      // Update kanban yang ada di request
      ...
      // Update kanban yang ada di database tetapi tidak ada di request -> js_ending_quantity = 0
      await prismaClient.kanban.updateMany({
        where: {
          code: { notIn: codes },
          js_ending_quantity: { not: 0 },
        },
        data: { js_ending_quantity: 0 },
      });

      logger.info("Kanban(s) not in request updated to 0");
