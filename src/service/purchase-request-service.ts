...
import {
  ...
  PurchaseRequestRawEntry,
  SearchPurchaseRequestRequest,
  toPurchaseRequestResponse,
} from "../model/purchase-request-model";
import {
  PurchaseRequestDetailRawEntry,
  CreatePurchaseRequestDetailRequest,
} from "../model/purchase-request-detail-model";
...
export class PurchaseRequestService {
  static async create(filePath: string) {
    ...
    for (let i = 5; i < data.length - 1; i++) {
      ...
      if (isPurposeOnly && purchaseRequestDetails.length > 0) {
        purchaseRequestDetails[purchaseRequestDetails.length - 1][
          "Purpose"
        ] += ` ${objDetail["Purpose"]}`;
        continue;
      }
      ...
      if (isContinuationRow && purchaseRequests.length > 0) {
        objDetail["PR No."] =
          purchaseRequests[purchaseRequests.length - 1]["PR No."];
      } else {
    ...
    const formattedResult: CreatePurchaseRequestRequest[] =
      purchaseRequests.map((entry: PurchaseRequestRawEntry) => {
        return {
    ...
    const detailFormattedResult: CreatePurchaseRequestDetailRequest[] =
      purchaseRequestDetails.map((entry: PurchaseRequestRawEntry) => {
        return {
          pr_number: parseString(entry["PR No."]),
    ...
    try {
      // Validasi data
      let createRequest = Validation.validate(
        PurchaseRequestValidation.CREATE,
        formattedResult
      ).filter((item) => item.pr_number?.startsWith("75"));

      let createRequestDetail = Validation.validate(
        PurchaseRequestDetailValidation.CREATE,
        detailFormattedResult
      ).filter(
        (item) =>
      ...
      const filteredRequest = createRequest.filter(
      ...
      if (filteredRequest.length === 0) {
        logger.error(
          `All PR numbers in file ${filePath} already exist in database`
        );
      } else if (filteredRequest.length !== createRequest.length) {
        logger.warn(
          `Some PR numbers in file ${filePath} already exist in database`
        );
        existingNumbers.forEach((pr) =>
          logger.warn(`PR Number ${pr} already exists in database`)
        );
      }
      ...
      await prismaClient.$transaction([
        ...
        prismaClient.purchaseRequestDetail.createMany({
          data: createRequestDetail,
        }),
      ]);
      ...
      logger.error(
        `Error while creating purchase request and details: ${error}`
      );
      throw new Error(
        `Error while creating purchase request and details: ${error}`
      );
  ...
  static async get(
    request: SearchPurchaseRequestRequest
  ): Promise<Pageable<PurchaseRequestResponse>> {
    const searchRequest = Validation.validate(
      PurchaseRequestValidation.SEARCH,
      request
    );
