...
import {
  ReminderResponse,
  toReminderResponse,
  SearchReminderRequest,
} from "../model/reminder-model";
import { Validation } from "../validation/validation";
import { ReminderValidation } from "../validation/reminder-validation";

export class ReminderService {
  static async get(
    request: SearchReminderRequest
  ): Promise<Pageable<ReminderResponse>> {
    const searchRequest = Validation.validate(
      ReminderValidation.SEARCH,
      request
    );
    ...
    const data = reminders.map((kanban) => {
      ...
      if (poDetails.length > 0) {
        const ordered = poDetails.find(
          (po) => po.status === "Partial Delivery"
        );
        if (ordered) {
